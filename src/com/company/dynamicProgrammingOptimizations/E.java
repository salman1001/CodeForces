...
public class E {
    public static void main(String[] args) {
        ...
        segmentTree.update(n,n,1);

        for (int i = n-k; i >= 0; i--) {
            int index = find_index(arr, i+k-1, arr[i]+d);
            if (index == i+k-1) continue;
            long val= segmentTree.rsq(i+k, index);
            if (val > 0) val = 1;
            segmentTree.update(i,i, val);
        }
        res = segmentTree.rsq(0,0);
        if (res == 0) {
    ...
    static int find_index(int arr[], int st, int val) {
        int l = st, r = arr.length-1;
        int ans = arr.length;
        while (l <= r) {
            int mid = l + (r-l)/2;
            if (arr[mid] > val) {
