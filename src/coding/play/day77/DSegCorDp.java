...
public class DSegCorDp {
    ...
    static class AnsPair{
        ...
        public AnsPair(int max, int index) {
            this.max = max;
            this.index = index;
        }
    }
    ...
    static class PairCor{
        int l;
        int r;

        public PairCor(int l, int r) {
            this.l = l;
            this.r = r;
        }
    }
    ...
    static PairCor[] pairCors=new PairCor[1000005];
    static long[] arr=new long[1000005];
    static ArrayList<ArrayList<PairCor>> ivv=new ArrayList<>();
    public static void main(String[] args) {
        ...
        for (int i=0;i<m;i++){
            ...
            coOrdinates.add(r);
            arr[i]=--x;
            pairCors[i]=new PairCor(l,r);
        }
        ArrayList<Integer> listOFcor=new ArrayList<>();
        for (Integer a:coOrdinates)  listOFcor.add(a);

        HashMap<Long,Long> hashMap=new HashMap();
        for (int i=0;i<listOFcor.size();i++){
            hashMap.put((long)listOFcor.get(i),(long)i);
        }

        for (int i=0;i<m;i++){
            pairCors[i].l=Math.toIntExact(hashMap.get( pairCors[i].l));
            pairCors[i].r=Math.toIntExact(hashMap.get( pairCors[i].r));
          //  ivv[i]=pairCors[i];
         //   ivv.add(Math.toIntExact(arr[i]),pairCors[i]);
            ivv.get((int) arr[i]).add(pairCors[i]);

        }
        long[] arr11 =new long[listOFcor.size()];
        SegmentTree segmentTree=new SegmentTree(arr11);
        AnsPair ansPair=new AnsPair(0,-1);
        for (int i=0;i<n;i++){
            AnsPair cur=new AnsPair(0,1);
           for (PairCor a:ivv.get(i)){
