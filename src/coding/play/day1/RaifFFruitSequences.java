...
import com.company.dynamicProgrammingOptimizations.E;

import java.util.Arrays;
import java.util.Scanner;

public class RaifFFruitSequences {
    ...
    public static void main(String[] args) {
        ...
        for (int i=0;i<n;i++){
            if (s.charAt(i)=='0'){
                ...
                long len=i-pontsRunning+1;
                long val=segmentTree.q(0,0,n-1,len);
                segmentTree.update((int) val,i,1);
                rightPoint+=i-val+1;
    ...
    public static class SegmentTree {
        ...
        public long q(int i,int tl,int tr,long val){
            propagate(i);
            if (tl==tr) return tl;
            int mid=tl+tr/2;
            propagate(2*i+1);
            if (rMinQ(0,2*i+1)<val) return q(2*i+1,tl,mid,val);
            return q(2*i+2,mid+1,tr,val);
