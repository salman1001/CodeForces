...
public class ZFrog {
    ...
    public static void main(String[] args) {
        ...
        int[] arr= fastScanner.readArray(n);
        long[] dp=new long[n+5];
        dp[0]=0;
        ConvexHullTrickMin convexHullTrickMin=new ConvexHullTrickMin();
        convexHullTrickMin.addLine(-2*arr[0],(long) Math.pow(arr[0],2));
        for (int i=1;i<n;i++){
            long query=convexHullTrickMin.query(arr[i]);
            dp[i]=c+((long) Math.pow(arr[i],2))+query;
            convexHullTrickMin.addLine(-2*arr[i],dp[i]+((long) Math.pow(arr[i],2)));
        }
...
//        for(int i = 0; i < n; i++) b[i] = sc.nextInt();
        ...
        static class InputReader {
            InputStream in;             private byte[] buf = new byte[16384];
            private int    curChar;
            private int    numChars;
InputReader(InputStream in) { this.in = in; }
